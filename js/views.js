...
function teamLink(name) {
  ...
  return `<a href="/teams/${slug}" data-link>${escapeHtml(name)}</a>`;
}

function playerLink(id) {
  const p = LEAGUE.playersById[id];
  if (!p) return escapeHtml(id);
  return `<a href="/player/${slugify(p.displayName)}" data-link>${escapeHtml(p.displayName)}</a>`;
}
...
function matchUrl(homeName, awayName) {
  return `/match/${slugify(homeName)}-${slugify(awayName)}`;
}

function weekUrl(n) { return `/week/${n}`; }
...
function homeView() {
  ...
  return `
    ...
    <div class="section">
      <div class="section-head"><h2>${t('home.team_rankings')}</h2><a class="view-all" href="/ranking/teams" data-link>${t('home.full_table')}</a></div>
      ${teamRankTable(teamStandings, false)}
    </div>

    <div class="section">
      <div class="section-head"><h2>${t('home.player_rankings')}</h2><a class="view-all" href="/ranking/players" data-link>${t('home.full_table')}</a></div>
      ${playerRankTable(playerStandings, false)}
    </div>

    <div class="section">
      <div class="section-head"><h2>${t('home.clearances')}</h2><a class="view-all" href="/ranking/clearances" data-link>${t('home.full_table')}</a></div>
